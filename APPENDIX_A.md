# Appendix A — Git & GitHub for People Who Skipped Them
...
## A.2 Install Git (once per machine)
...
If you see something like `git version 2.43.0`, you are done — skip to A.3. If you get "command not found," install it:
...
Use the same email you will use for GitHub (A.4); it links your commits to your account.

---

## A.3 Your first repo: the six commands you actually need
...
> ```
>
> Git will then pretend those files do not exist for commit purposes. Every chapter's repo ships a `.gitignore`; if you start a fresh project, write one *before* your first commit. Appendix B explains why committing a `.env` can leak an API key to the whole internet.

---

## A.4 Create a GitHub account and get the book's code
...
1. Go to [github.com](https://github.com) and click **Sign up**. Use the same email you put in `git config` (A.2).
...
## A.5 Push your own work to GitHub
...
## A.6 Authentication: the part that trips everyone up
...
Treat a PAT exactly like an API key (Appendix B): it is a secret, it grants access to your repos, and you should never paste it into a file you might commit.
...
- **"Authentication failed" on push.** You used your account password instead of a token, or you have not run `gh auth login`. See A.6.
- **You committed a `.venv/` or a `.env` by accident.** Add it to `.gitignore`, then run `git rm -r --cached .venv` (or the filename) and commit again. If a *secret* was committed, treat it as leaked: rotate the key immediately (Appendix B, B.7).
