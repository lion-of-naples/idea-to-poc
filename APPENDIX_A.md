...
If you see `Python 3.10.x` or higher, you are set — skip to A.3. If you see `Python 2.7`, an "unknown command" error, or a version below 3.10, install a current Python (A.2).

> **`python` vs `python3` — the classic confusion.** On many Macs and Linux systems, the bare command `python` either does not exist or points at an ancient Python 2. The command that reliably means "modern Python" is `python3`, which is why every example in this book uses it. On Windows the command is usually just `python`. Rule of thumb: try `python3` first; if that is "not found," try `python`. Once you are *inside* an activated virtual environment (A.3), plain `python` becomes safe to use because it points at that environment's interpreter.
...
This creates a folder named `.venv` in your current directory. (`-m venv` means "run Python's built-in `venv` module"; `.venv` is just the conventional folder name — the leading dot keeps it tidy and it is always `.gitignore`'d, per Appendix B.3.)
...
## A.5 Running the tools and the tests
...
Putting A.3–A.5 together, here is the exact sequence for following any chapter from a fresh clone (Appendix B explains the `git clone`):
